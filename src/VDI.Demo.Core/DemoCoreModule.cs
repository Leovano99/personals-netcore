...
using VDI.Demo.Emailing;
using VDI.Demo.Features;
using VDI.Demo.Friendships;
using VDI.Demo.Friendships.Cache;
using VDI.Demo.Localization;
using VDI.Demo.MultiTenancy;
using VDI.Demo.MultiTenancy.Payments.Cache;
...
namespace VDI.Demo
{
    ...
    public class DemoCoreModule : AbpModule
    {
        public override void PreInitialize()
        {
            ...
            //Configuration.ReplaceService<IMailKitSmtpBuilder, E3MailKitSmtpBuilder>();

            Configuration.ReplaceService(typeof(IEmailSenderConfiguration), () =>
            {
                Configuration.IocManager.IocContainer.Register(
                    Component.For<IEmailSenderConfiguration, ISmtpEmailSenderConfiguration>()
                             .ImplementedBy<DemoSmtpEmailSenderConfiguration>()
                             .LifestyleTransient()
                );
            });
            ...
            {
                cache.DefaultSlidingExpireTime = TimeSpan.FromMinutes(DemoConsts.PaymentCacheDurationInMinutes);
            });
        }
